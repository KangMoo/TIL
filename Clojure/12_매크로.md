...
매크로는 코드가 실행되기 전에 먼저 실행되서 코드를 만들고, 그 다음에 코드가 실행되기 때문에 결과는 같게 나왔다.

먼저 예제에서 리턴 값이 `id`심폴인 경우 `id`코드를 생성하고 `id`를 평가해 `1`이 나왔다.

마찬가지로 `(+ 2 3)` 리스트가 리턴되었고 이것을 평가하면 첫번째 항목이 함수이기 때문에 결과인 `5`가 나온 것이다.
...
매크로는 코드가 컴파일 되기 전에 실행되어 모든 매크로의 결과가 코드로 바뀐 후에 코드가 컴파일된다.
...
평가되면 매크로가 실행된 결과가 넘어가기 때문에 매크로가 코드로 바뀐 결과를 제대로 확인할 수 없다

## 매크로로 코드 만들기

위에서 알아 본 것처럼 매크로는 코드가 실행되기 전에 모두 실행되어 결과가 코드로 바뀌기 때문에 패턴화 되는 코드를 만드는데 사용할 수 있다.

하지만 처음에 얘기한 것처럼 함수로 작성할 수 있다면 매크로보다 함수로 작성하는 것이 더 좋다.

`(str "Hi! I'm " 입력값)`형태의 코드를 만드는 매크로를 만들어보자
...
`(code-greeting "eunmin")`매크로의 실행결과는 컴파일되기 전에 `(str "Hi! I'm" "eunmin")`이라는 코드로 대치되고 그다음 코드가 평가돼서 "Hi! I'm eunmin"이라는 결과가 나온다.

위 예에서 매크로의 결과가 코드여야 하기 때문에 리스트에 코드를 담았다

리스트에 담지 않고 `(str "Hi! I'm" me)`라고 했다면 매크로의 결과가 코드 형태가 아닌 `str`의 값이 된다
...
만들어진 코드의 모양도 `list`로 만든 경우 `+`함수가 평가된 함수 값이 들어 있고 `'`표시로 만든 경우 `+`가 평가되지 않고 `+`심볼이 그대로 나온 것을 알 수 있다

만약에 `list`로 만들면서 `+`가 평가되지 않게 하려면 `+`앞에만 `'`를 붙여주면 된다
...
이번에는 `'`표시로 앞서 만든 `codec-greeting` 매크로를 만들어보자
...
매크로의 만든 코드가 `(str "Hi! I'm " "eunmin")` 대신 `(str "Hi! I'm " me)`가 대신 나왔다

코드를 평가한 결과도 오류가 발생했다. `me`라는 심볼은 현재 네임스페이스인 `user`에 없는 Var이기 때문이다.

왜 이런 일이 발생한 건가?

`' `표시는 안에 있는 모든 코드를 평가하지 않기 때문에 `me` 값이 평가되지 않고 심볼 그대로 리턴되었기 때문이다.

`list`를 사용하면 앞에 붙은 `list` 때문에 코드가 간결해 보이지 않고 또 안쪽에 평가되어야 할 심볼과 그렇지 않은 심볼이 `'` 표시로 되기 때문에 잘 눈에 띄지 않는다.

클로저에 매크로를 템플릿처럼 사용할 수 있는 기능을 쓰려면 `list`를 쓰지 않고 보기 좋게 작설할 수 있다
...
`list`로 만들었을 때는 안쪽의 코드가 기본적으로 평가되고 `'` 표시를 해서 평가하지 않도록 했고, ~표시로 평가하도록 했다.



## 매크로에서 심볼 사용하기 

위에서 본 것처럼 ` 표시로 매크로를 만들면 심볼 앞에 네임스페이스가 붙는다
...
이런 경우에 네임스페이스가 앞에 붙지 않도록 하기 위해 꼼수를 써야 한다

이상해보이지만 `a`를 심볼로 생성하기 위해 평가되지 않는 기호 `'`를 붙이고 그 앞에 또다시 `~` 평가하게 되면 결국 `a`심볼을 얻을 수 있다
...
매크로 안에 심볼을 만들어 써야 하는 경우 `심볼#1`이라는 구문을 쓰면 겹치지 않는 심볼을 만들 수 있다.
...
시퀀스 형태의 값을 순서대로 대치해주는 `~@심볼` 이라는 기호가 있다. 예를 들어 `[1 2 3 ]`이라는 값으로 들어오는 매크로의 파라미터가 있다면 `~@심볼` 로 풀어서 `1 2 3`으로 대치해줄 수 있다
...
클로저에서 제공하는 여러 가지 기능 중 매크로로 지원하는 것들이 많이 있다

먼저 함수 값에 심볼을 바인딩하는 `defn`은 매크로다
