...
public 접근 제어자를 사용하여 선언된 클래스 멤버는 외부로 공개되며, 해당 객체를 사용하는 프로그램 어디서나 직접 접근할 수 있다. 자바 프로그램은 public 메서드를 토해서만 해당 객체의 private 멤버에 접근할 수 있다. 따라서 public 메서드는 private 멤버와 프로그램 사이의 인터페이스 역할을 수행한다고 할 수 있다.

![public 접근 제어자](./image/02_2.png)
...
자바에서는 클래스 및 클래스 멤버의 저근 제어의 기본값으로 default 접근 제어를 별도로 명시하고 있다. 이러한 default를 위한 접근 제어자는 따로 존제하지 않으며, 접근 제어자가 지정되지 않으면 자동적으로 default 접근 제어를 가지게 된다. default 접근 제어를 가지는 멤버는 같은 클래스의 멤버와 같은 패키지에 속하는 멤버에서만 접근할 수 있다.

![default 접근 제어](./image/02_03.png)
...
자바 클래스는 private멤버로 정보를 은닉하고, public 멤버로 사용자나 프로그램과의 인터페이스를 구축한다. 여기에 부모 클래스와 관련된 접근 제어자가 하나 더 존재한다. protected 멤버는 부모 클래스에 데해서는 public 멤버처럼 취급되며, 외부에서는 privsate 멤버처럼 취급된다.
...
![protected 접근 제어자](./image/02_3.png)
